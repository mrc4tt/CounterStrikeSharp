// Vendored from hl2sdk-cs2 @ 5f891c90 (public/engine/IEngineSound.h), which upstream
// deleted in "Clean up unused files" (363855e8). CounterStrikeSharp still binds
// IEngineSound for the PrecacheSound / IsSoundPrecached / GetSoundDuration
// natives, so the declaration is kept verbatim -- the vtable layout must match
// the engine exactly, so do not reorder or edit the virtuals.
...
#include "utlvector.h"
#include "core/cs2_sdk/compat/SndInfo.h"
...
abstract_class IEngineSound
{
  public:
    ...
    virtual int EmitSound(IRecipientFilter & filter, int iEntIndex, int iChannel, const char* pSoundEntry, unsigned int nSoundEntryHash,
                          const char* pSample, float flVolume, float flAttenuation, int nSeed, int iFlags = 0, int iPitch = PITCH_NORM,
    ...
    virtual int EmitSound(IRecipientFilter & filter, int iEntIndex, int iChannel, const char* pSoundEntry, unsigned int nSoundEntryHash,
                          const char* pSample, float flVolume, soundlevel_t iSoundlevel, int nSeed, int iFlags = 0, int iPitch = PITCH_NORM,
                          const Vector* pOrigin = NULL, const Vector* pDirection = NULL, CUtlVector<Vector>* pUtlVecOrigins = NULL,
                          bool bUpdatePositions = true, float soundtime = 0.0f, int speakerentity = -1) = 0;

    virtual void EmitSentenceByIndex(
        IRecipientFilter & filter, int iEntIndex, int iChannel, int iSentenceIndex, float flVolume, soundlevel_t iSoundlevel, int nSeed,
        int iFlags = 0, int iPitch = PITCH_NORM, const Vector* pOrigin = NULL, const Vector* pDirection = NULL,
        CUtlVector<Vector>* pUtlVecOrigins = NULL, bool bUpdatePositions = true, float soundtime = 0.0f, int speakerentity = -1) = 0;
