...
const config = z.object({
    ...
    // Either a full `sftp://host:port` URL, or the bare host that
    // eng/update-schema.ts already uses (then SFTP_PORT applies), so the
    // repository's existing SFTP_HOST secret serves both workflows.
    SFTP_HOST: z.string().regex(/^(sftp:\/\/)?[a-zA-Z0-9.\-\[\]:]+$/),
    SFTP_PORT: z.preprocess((value) => value === "" ? undefined : value, z.string().regex(/^\d+$/).default("2022")),
...
const sftpUrl = config.SFTP_HOST.startsWith("sftp://") ? config.SFTP_HOST : `sftp://${config.SFTP_HOST}:${config.SFTP_PORT}`;
