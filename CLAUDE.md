...
- `gamedata/gamedata.json` maps logical function names to per-platform Windows/Linux byte signatures and vtable offsets. **`configs/.../gamedata.json` is the single source of truth and the only git-tracked copy.** The copies under `build/...` and `out/...` are gitignored build artifacts, regenerated from `configs/` by CMake's `copy_directory` step (`CMakeLists.txt`) — edit `configs/`, never the artifacts. (They only drift if you hand-edit an artifact or ship a stale build, not from normal editing.)
